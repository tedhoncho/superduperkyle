...
// --- Notify me (Coming Soon email capture) ---
...
app.post('/api/notify-me', (req, res) => {
  try {
    ...
    if (!project.comingSoon) {
      return res.status(400).json({ error: 'This one is already available — go ahead and buy it!' });
    }

    const isNew = db.addNotifySignup({ projectId, email });
    res.json({ ok: true, alreadySignedUp: !isNew });
  } catch (err) {
    console.error('[notify-me] error:', err);
    res.status(500).json({ error: 'Something went wrong saving your email.' });
  }
});
